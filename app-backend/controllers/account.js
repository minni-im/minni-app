...
export default (app) => {
      ...
      Account.getListForUser(req.user.id).then((accounts) => {
        ...
        if (
          ...
          !accounts
            ...
            .includes(sanitizeName(accountName))
        ) {
          res.redirect("/");
          return;
        }
        ...
        res.render("chat", {
          accounts: accounts.map((account) =>
            account.toAPI(req.user.id === account.adminId)
          ),
        });
      });
    }
  ...
  app.io.route("accounts", {
    ...
    get(req, res) {
      ...
      Account.findById(accountId).then(
        (account) => {
          if (account === false) {
            return res.json({
              ok: false,
        ...
        .then((accounts) => {
          res.json({
            ...
            accounts: accounts.map((account) => {
              ...
              console.log(
                `'${req.user.id}' joining account:${account.id} (${account.name})`
              );
      ...
      account
        ...
        .then((savedAccount) => {
          ...
          const room = new Room({
            name: "Central Plaza",
            topic:
              "This is room is for team-wide communication. All team members can access this room.",
            type: TYPE.INITIAL,
            adminId: user.id,

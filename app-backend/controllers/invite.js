...
export default (app) => {
  ...
  /* =API routes= */
  app.get("/api/accounts/:accountId/invites", requireLogin, requireValidAccount, (req) => {
  ...
  app.io.route("invites", {
    accept(req, res) {
      ...
      Invite.findByToken(inviteToken).then((invite) => {
        if (!invite.expired) {
          ...
          Account.findById(accountId)
            ...
            .then((savedAccount) => {
              invite.usage += 1;
              return invite.save().then(savedInvite => [savedAccount, savedInvite]);
            })
            .then(([savedAccount, savedInvite]) => {
              savedAccount = savedAccount.toAPI(false);
              app.io.in(savedAccount.id).emit("account:join", {
                user: req.user.toAPI(),
    ...
    list(req, res) {
      ...
      Invite.where("accountId", { key: account.id }).then((invites) => {
        res.json({
          ok: true,
    ...
    create(req, res) {
      ...
      Invite.generateToken(userId, accountId, maxAge, maxUsage).then(invite =>
        res.status(201).json({
          ok: true,
          accountId,
          invite: invite.toAPI(),
    ...
    delete(req, res) {
      ...
      Invite.findByToken(inviteToken).then(invite => invite.remove()).then((invite) => {
        res.json({
          ok: true,
          invite: invite.toAPI()
        });
